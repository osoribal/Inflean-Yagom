...
class TodoViewController: UIViewController {
    ...
    private var editButton: UIBarButtonItem {
        let button: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: .edit, target: self, action: #selector(touchUpEditButton(_:)))
        return button
    }
    
    /// 취소 - 네이게이션 바 버튼
    private var cancelButton: UIBarButtonItem {
        let button: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(touchUpCancelButton(_:)))
        return button
    }
    
    /// 완료 - 네비게이션 바 버튼
    private var doneButton: UIBarButtonItem {
        let button: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(touchUpDoneButton(_:)))
        return button
    }
    ...
    @objc private func touchUpDoneButton(_ sender: UIBarButtonItem) {
        ...
        guard let title: String = self.titleField.text, title.isEmpty == false else {
            self.showSimpleAlert(message: "제목은 꼭 작성해야합니다", cancelHandler: {(action: UIAlertAction) in self.titleField.becomeFirstResponder()})
            return
        }
        
        // 새로운 todo 생성
        let todo : Todo
        todo = Todo(title: title, due: self.dueDatePicker.date, memo: self.memoTextView.text, shouldNotify: self.shouldNotifyswitch.isOn, id: self.todo?.id ?? String(Date().timeIntervalSince1970)) /// 유닉스 타임프템프를 할 일 고유 아이디로 활용
        ...
        if self.todo == nil {
            ...
            // 수정상태라면 저장을 완료하고 화면을 보기모드로 전화
            isSuccess = todo.save(comletion: {
                self.todo = todo
                self.mode = Mode.view
            })
        }
